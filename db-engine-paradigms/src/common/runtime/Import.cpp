...
struct ColumnConfigOwning {
   ...
   ColumnConfigOwning(string n, unique_ptr<algebra::Type>&& t, string t2 = "")
       : name(n), type(move(t)) {}
...
#define D(rt_type)                                                             \
   {                                                                           \
      ...
      MinMaxInfo m; \
      auto arr = data.data(); \
      for (size_t i=0; i<sz; i++) { arr[i].minmax(m); } \
      if (m.flags & MinMaxInfo::kVariableSize) { \
        assert(m.max_len > 0); \
        attr.varchar_data.reserve(sz + 4*1024); \
        for (size_t i=0; i<sz; i++) { \
          attr.varchar_data.emplace_back(varchar(arr, i, m.max_len)); \
        } \
      } \
      /* printf("%s [%f, %f]\n", path.c_str(), m.lo, m.hi); */ \
      attr.minmax = new MinMaxInfo(m); \
      return sz; \
   }
...
void parseColumns(runtime::Relation& r, std::vector<ColumnConfigOwning>& cols,
   ...
   int mkdir_res = mkdir((cachedir).c_str(), 0777);
   if (mkdir_res != 0) {
      if (errno != EEXIST) {
        throw runtime_error("Could not create dir 'cached': " + cachedir + " " + strerror(errno));
      }
   }
   for (auto& col : colsC)
   ...
   if (!allColumnsMMaped) {
      ...
      std::string fname {dir + "/" + fileName + ".tbl"};
      ifstream relationFile(fname);
      if (!relationFile.is_open())
         throw runtime_error("csv file not found: " + fname);
      string line;
...
void importTPCH(std::string dir, Database& db) {
   ...
   // part
   {
      ...
      rel.name = "part";
      auto columns =
          configX({{"p_partkey", make_unique<algebra::Integer>(), "i32"},
                   {"p_name", make_unique<algebra::Varchar>(55), "str"},
                   {"p_mfgr", make_unique<algebra::Char>(25), "str"},
                   {"p_brand", make_unique<algebra::Char>(10), "str"},
                   {"p_type", make_unique<algebra::Varchar>(25), "str"},
                   {"p_size", make_unique<algebra::Integer>(), "i32"},
                   {"p_container", make_unique<algebra::Char>(10), "str"},
                   {"p_retailprice", make_unique<algebra::Numeric>(12, 2), "i64"},
                   {"p_comment", make_unique<algebra::Varchar>(23), "str"}});
   ...
   // supplier
   {
      ...
      rel.name = "supplier";
      auto columns =
          configX({{"s_suppkey", make_unique<algebra::Integer>(), "i32"},
                   {"s_name", make_unique<algebra::Char>(25), "str"},
                   {"s_address", make_unique<algebra::Varchar>(40), "str"},
                   {"s_nationkey", make_unique<algebra::Integer>()},
                   {"s_phone", make_unique<algebra::Char>(15), "str"},
                   {"s_acctbal", make_unique<algebra::Numeric>(12, 2)},
                   {"s_comment", make_unique<algebra::Varchar>(101), "str"}});
   ...
   // partsupp
   {
      ...
      auto columns =
          configX({{"ps_partkey", make_unique<algebra::Integer>()},
                   ...
                   {"ps_comment", make_unique<algebra::Varchar>(199), "str"}});
   ...
   // customer
   {
      ...
      auto columns =
          configX({{"c_custkey", make_unique<algebra::Integer>()},
                   {"c_name", make_unique<algebra::Char>(25), "str"},
                   {"c_address", make_unique<algebra::Varchar>(40), "str"},
                   {"c_nationkey", make_unique<algebra::Integer>()},
                   {"c_phone", make_unique<algebra::Char>(15), "str"},
                   {"c_acctbal", make_unique<algebra::Numeric>(12, 2)},
                   {"c_mktsegment", make_unique<algebra::Char>(10), "str"},
