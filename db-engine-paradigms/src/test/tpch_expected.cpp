...
map3_t& q3_expected() {
   ...
   if (!expected.size()) {
      ...
      std::ofstream output(compat::path(__FILE__) + "/q3_expected.result");
      Attribute value;
      for (std::string line; std::getline(input, line);) {
         ...
         output << l_orderkey.value << "|" << o_orderdate.value << "|" << o_shippriority.value << "|" << sum.value << std::endl;
      }
   }
   return expected;
}
...
map9_t& q9_expected() {
  ...
  if (!expected.size()) {
    ...
    std::ofstream output(compat::path(__FILE__) + "/q9_expected.result");
    Attribute value;
    for (std::string line; std::getline(input, line);) {
       value.ptr = line.data();
       value.len = -1;
       ...
       output << nation << "|" << o_year.value << "|" << sum.value << std::endl;
    }
  }
  return expected;
}
...
map18_t& q18_expected() {
   ...
   if (!expected.size()) {
      ...
      std::ofstream output(compat::path(__FILE__) + "/q18_expected.result");
      Attribute value;
      for (std::string line; std::getline(input, line);) {
         value.ptr = line.data();
         value.len = -1;
         ...
         expected.emplace(std::make_tuple(name, c_custkey, o_orderkey,
                          ...
                          sum);
         output << name << "|" << c_custkey.value << "|" << o_orderkey.value << "|" <<o_orderdate.value << "|" << o_totalprice.value << std::endl;
      }
   }
   return expected;
}
